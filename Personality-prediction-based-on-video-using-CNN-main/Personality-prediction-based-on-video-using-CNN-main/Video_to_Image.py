...
### Testing data ###
FilePath = "E:/F Y P/Personality prediction using image processing/datasets/First Impressions V2 (CVPR'17)/test/test-1/testting data/"
...
while(len(os.listdir(FilePath)) > i):
        path1= FilePath + os.listdir(FilePath)[i]       
        cap = cv2.VideoCapture(path1)
        file_name = os.listdir(FilePath)[i]
        file_name = (file_name.split(".mp4"))[0]
        ## Creating folder to save all the 100 frames from the video
        try:
            if not os.path.exists("ImageData/testingData/" + file_name):
